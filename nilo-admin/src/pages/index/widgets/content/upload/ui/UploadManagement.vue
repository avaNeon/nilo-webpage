...
<template>
    <div class="upload-management">
        ...
        <el-card class="filter-card" shadow="never">
            <div class="filter-row">
                ...
                <el-cascader v-model="cascaderValue" :options="flatCategoryOptions"
                    :props="{ checkStrictly: true, emitPath: true }" placeholder="选择分类" class="filter-cascader"
                    @change="onCategoryChange" />
                <el-select v-model="selectedRecommend" placeholder="推荐状态" clearable class="filter-select">
                    <el-option v-for="opt in recommendOptions" :key="String(opt.value ?? 'all')" :label="opt.label"
                        :value="opt.value" />
                </el-select>
        ...
        <el-card class="table-card" shadow="never">
            <el-table v-loading="loading" :data="videoList" style="width: 100%" stripe border size="large"
                row-key="videoId" @sort-change="handleSortChange" @row-click="openPreview">
                <!-- 封面 -->
                ...
                <!-- 创建时间 / 最后更新时间 -->
                <el-table-column label="创建/更新时间" prop="lastUpdateTime" sortable="custom"
                    :sort-orders="['descending', 'ascending', null]" width="220" align="center">
                    <template #default="{ row }">
                        <div class="time-cell">
                            <div class="time-row">
                ...
                <el-table-column label="状态" prop="status" sortable="custom"
                    :sort-orders="['descending', 'ascending', null]" width="110" align="center">
                    <template #default="{ row }">
                ...
                <el-table-column label="推荐" width="100" align="center">
                    <template #default="{ row }">
                        <span v-if="row.recommendType !== undefined"
                            :class="['recommend-tag', getRecommendClass(row.recommendType)]">
                            {{ getRecommendLabel(row.recommendType) }}
                        </span>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" width="240" align="center" fixed="right">
                    <template #default="{ row }">
                        <div class="action-btns" @click.stop>
                            <el-button v-if="row.status === VideoStatusEnum.PendingReview" type="success" size="small"
                                link @click="handleReview(row, true)">
                                通过
                            </el-button>
                            <el-button v-if="row.status === VideoStatusEnum.PendingReview" type="danger" size="small"
                                link @click="handleReview(row, false)">
                                驳回
                            </el-button>
                            <el-button v-if="row.recommendType !== undefined" type="primary" size="small" link
                                @click="handleToggleRecommend(row)">
                                {{ row.recommendType === 1 ? "取消推荐" : "推荐" }}
                            </el-button>
                            <el-button v-if="row.status !== VideoStatusEnum.Transcoding" type="danger" size="small" link
