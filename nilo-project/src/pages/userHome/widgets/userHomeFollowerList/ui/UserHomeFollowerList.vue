...
const { userList, loading, noMore, loadFollowerList } = useUserHomeFollowerList();
...
<template>
  <div class="content">
    <div class="main-content">
      <div class="label">
        <span class="theme-text">粉丝列表</span>
        <span class="counter-text" v-if="!loading">({{ userList.length }})</span>
      </div>

      <div v-if="userList.length > 0" class="user-items">
        <UserItem v-for="user in userList" :key="user.userId" :user="user" list-type="follower" @toggle="onToggle" />
      </div>
      ...
      <div class="pagination" v-if="userList.length > 0">
        <el-pagination layout="prev, pager, next" :page-size="10"
          ...
          @current-change="(newPageNo: number) => loadFollowerList(newPageNo)" :current-page="1" />
      </div>
    </div>
  </div>
</template>
