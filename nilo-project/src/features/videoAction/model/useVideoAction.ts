...
export function useVideoAction() {
    ...
    async function getVideoAction() {
        ...
        if (loginState.loginState === false) { return }

        // else fetch video action status for the current user
        const result = await VideoActionApi.getVideoAction(route.params.videoId as string)
    ...
    async function doVideoAction(actionType: number, coinAmount?: number) {
    ...
    watch(() => loginState.loginState, (loggedIn) => {
        if (loggedIn) {
            getVideoAction()
        }
    })
