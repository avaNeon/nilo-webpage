...
export function useAuthForm() {
  ...
  function submit() {
    formDataRef.value?.validate(async (valid: boolean) => {
      ...
      // 调用 API
      let tokenUserInfo: TokenUserInfo | null = null;
      if (inLogin.value) {
        tokenUserInfo = await AccountApi.login(params, errorCallback);
      } else {
        await AccountApi.register(params, errorCallback);
      }
